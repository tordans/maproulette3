...
describe('OSM Service Functions', () => {
  ...
  test('fetchOSMData should handle 400 Bad Request error', async () => {
  ...
  test('fetchOSMData should handle 509 Bandwidth Exceeded error', async () => {
  ...
  test('fetchOSMData should resolve with parsed XML when fetch is successful', async () => {
  ...
  test('fetchOSMData should reject with fetchFailure error on fetch failure', async () => {
    global.fetch.mockRejectedValueOnce(new Error('Network Error'));
    await expect(fetchOSMData('0,0,1,1')).rejects.toEqual(AppErrors.osm.fetchFailure);
  });

  test('fetchOSMData should resolve with parsed XML when response is empty but still successful', async () => {
  ...
  test('fetchOSMData should handle valid XML with unexpected structure', async () => {
  ...
  test('fetchOSMElement should handle 400 Bad Request error', async () => {
    global.fetch.mockResolvedValueOnce({ ok: false, status: 400 });
    await expect(fetchOSMElement('node/12345')).rejects.toEqual(AppErrors.osm.requestTooLarge);
  });

  test('fetchOSMElement should reject with fetchFailure error on fetch failure', async () => {
    global.fetch.mockRejectedValueOnce(new Error('Network Error'));
    await expect(fetchOSMElement('node/12345')).rejects.toEqual(AppErrors.osm.fetchFailure);
  });

  test('fetchOSMElement should correctly handle different OSM element types', async () => {
  ...
  test('fetchOSMElement should handle invalid element ID format gracefully', async () => {
  ...
  test('fetchOSMElement should handle large XML responses', async () => {
  ...
  test('fetchOSMElement should handle 404 Not Found error', async () => {
    global.fetch.mockResolvedValueOnce({ ok: false, status: 404 });
    await expect(fetchOSMElement('node/12345')).rejects.toEqual(AppErrors.osm.elementMissing);
  });

  test('fetchOSMElement should handle 509 Bandwidth Exceeded error', async () => {
    global.fetch.mockResolvedValueOnce({ ok: false, status: 509 });
    await expect(fetchOSMElement('node/12345')).rejects.toEqual(AppErrors.osm.bandwidthExceeded);
  });

  test('fetchOSMElement should return JSON when asXML is false', async () => {
    const mockXML = `<osm><node id="1" lat="1.1" lon="1.1"></node></osm>`;
    global.fetch.mockResolvedValueOnce({
      ok: true,
      text: () => Promise.resolve(mockXML),
  ...
  test('fetchOSMElement should return XML when asXML is true', async () => {
    const mockXML = `<osm><node id="1" lat="1.1" lon="1.1"></node></osm>`;
    global.fetch.mockResolvedValueOnce({
      ok: true,
      text: () => Promise.resolve(mockXML),
  ...
  test('fetchOSMElement should handle empty responses', async () => {
    global.fetch.mockResolvedValueOnce({
      ok: true,
  ...
  test('fetchOSMElementHistory should map valid changesets correctly', async () => {
    const mockHistory = {
      elements: [
        { id: '1', changeset: '100' },
        { id: '2', changeset: '101' }
  ...
  test('fetchOSMElementHistory should not modify history entries for missing changesets', async () => {
    const mockHistory = {
      elements: [
        { id: '1', changeset: '999' },
        { id: '2', changeset: '888' }
  ...
  test('fetchOSMElementHistory should map changesets correctly when changesets are present in changesetMap', async () => {
    const mockHistory = {
      elements: [
        { id: '1', changeset: '100' },
        { id: '2', changeset: '101' }
  ...
  test('fetchOSMElementHistory should not map changesets if they are not in changesetMap', async () => {
    const mockHistory = {
      elements: [
        { id: '1', changeset: '999' },
        { id: '2', changeset: '888' }
  ...
  test('fetchOSMElement should reject with custom error if error object has id and defaultMessage properties', async () => {
  ...
  test('fetchOSMElement should reject with fetchFailure if error object does not have id and defaultMessage properties', async () => {
  ...
  test('fetchOSMElementHistory should handle empty changeset map', async () => {
    const mockHistory = {
      elements: [
        { id: '1', changeset: '100' },
        { id: '2', changeset: '101' }
  ...
  test('fetchOSMElementHistory should not attempt to map changesets if asXML is false', async () => {
    const mockHistory = {
      elements: [
        { id: '1', changeset: '100' },
        { id: '2', changeset: '101' }
  ...
  test('fetchOSMChangesets should return changesets correctly', async () => {
  ...
  test('fetchOSMUser should return user data correctly', async () => {
    const mockXML = '<osm user="user123" display_name="John Doe" />';
    global.fetch.mockResolvedValueOnce({
      ok: true,
      text: () => Promise.resolve(mockXML),
    });
